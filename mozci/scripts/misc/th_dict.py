...
buildernames = filter_buildernames([" try "], ["hg clone"], list_builders()) + \
    filter_buildernames(["-try-"], ["hg clone"], list_builders()) + \
    filter_buildernames(["_try_"], ["hg clone"], list_builders())
...
def write_revision_graph():
    """Write the json graph to a file."""
    with open('th_try_graph.json', 'w') as f:
        graph = allthethings_th
        json.dump(graph, f, sort_keys=True, indent=4, separators=(',', ': '))

write_revision_graph()
# job = {
